...
public class ParseCreatureInput {
    ...
    private static Pattern p4 = Pattern.compile("^ *([-+0-9.]+) +([^()\" ]+) +([^()\"]+)(?: +\\(([^()\"]*)\\))? *$");
        ...
        for (final String input2 : input1s) {
            ...
            for (final String input3 : Arrays.asList(input2.split(";;", -1)).stream().map(x -> x.trim()).toList()) {
                ...
                if (matcher.matches()) {
                    ...
                    if (stat != null) {
                        stat.addInitialValue(new Value(true).source("input"));
                        continue;
                    }
                }
                ...
                if (matcher.matches()) {
                    ...
                    stat.addInitialValue(new Value(value).source(source));
                    continue;
                }
                ...
                matcher = p4.matcher(input3);
                if (matcher.matches()) {
                    final String name = matcher.group(3).trim();
                    final String value = matcher.group(1);
                    final String type = matcher.group(2);
                    final String source = Optional.ofNullable(matcher.group(4)).orElse("input").trim();
                    final ValuedStat stat = stats.get(name);
                    if (stat != null) {
                        stat.addInitialValue(new Value(value).type(type).source(source));
                        continue;
                    }
                }
